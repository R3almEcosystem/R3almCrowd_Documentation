---
title: "R3almCrowd_Documentation — System Design"
...
A artifact moves through **capture → classify → review → version → retrieve**. Each transition must define its actor, prerequisites, validation, durable state change, emitted evidence, retry behavior, and user-visible result.
